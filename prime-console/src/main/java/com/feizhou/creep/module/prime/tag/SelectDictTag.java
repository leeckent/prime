package com.feizhou.creep.module.prime.tag;

import com.feizhou.creep.constant.factory.ConstantFactory;
import com.feizhou.creep.enums.SfyxEnum;
import com.feizhou.creep.framework.core.util.SpringContextHolder;
import com.feizhou.creep.helper.enums.EnumIntType;
import com.feizhou.creep.helper.enums.Enumeration;
import com.feizhou.creep.module.prime.service.dict.CustomerDictService;
import com.feizhou.creep.module.system.model.SystemDict;
...
public class SelectDictTag extends GeneralVarTagBinding {
    ...
    private CustomerDictService dictService = SpringContextHolder.getBean("customerDictService");
    ...
    private List<Enumeration> getDictEnums(String key) {
        ...
        List list = null;
        if (key.equals(SfyxEnum.class.getSimpleName())) {
            list = SfyxEnum.getAllEnum();
        }
        if (list != null && list.size() != 0) {
            result = new ArrayList<>();
            for (Object obj : list) {
                EnumIntType element = (EnumIntType) obj;
                Enumeration enumeration = new Enumeration(element.intValue()
                        + "", element.getName());
    ...
    private List<Enumeration> getDictList() {
        ...
        List<Enumeration> result = null;
        List<SystemDict> list = ConstantFactory.me().findInDict(dictName);
        if (list != null && list.size() != 0) {
            result = new ArrayList<>();
